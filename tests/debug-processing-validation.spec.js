...
test.describe('Debug Processing Validation', () => {
  test('debug why processing options validation is not working', async ({ page }) => {
    console.log('🚀 Starting debug processing validation test...');
    ...
    await page.screenshot({ path: 'debug-validation-1-current-state.png' });
    ...
    if (availableCount > 0) {
      ...
      if (cutToSizeCount > 0) {
        ...
        await cutToSizeButton.first().screenshot({ path: 'debug-validation-2-cut-to-size-button.png' });
        
        // Step 12: Click on "Cut to Size" and watch for debug logs
        ...
        await page.screenshot({ path: 'debug-validation-3-after-click.png' });
        ...
        if (modalCount > 0) {
          ...
          await modal.screenshot({ path: 'debug-validation-4-option-modal.png' });
          ...
          if (inputCount > 0) {
            ...
            await page.screenshot({ path: 'debug-validation-5-input-filled.png' });
            ...
            await page.screenshot({ path: 'debug-validation-6-final-state.png' });
